// The look of the station: the classic flat-shaded one, or Kenney's space kits over the same floor
// plan. The model, the walks and the names on every node are the same either way; only what a
// thing is drawn as changes.
...
enum Theme: String, CaseIterable, Identifiable {
    ...
    /// The theme the scene draws with, set from the config when the scene is built and on every change.
    static var active: Theme = .classic
    static var isKenney: Bool { active == .kenney }
}

/// Kenney's models (CC0, www.kenney.nl): the Space Kit and the Modular Space Kit, each loaded once
/// from the bundle and cloned out with materials of its own, so a tint touches one copy.
enum Kit {
