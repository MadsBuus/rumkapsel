...
struct SettingsView: View {
    ...
    private var pipeline: some View {
        page {
            ...
            Text("Merged pull requests go to storage. A release into the staging branch moves them to the test deck. A release into the production branch loads the rocket. These names are looked for first; each repository's own branches decide. Without a staging branch a repository skips the deck and loads its rocket from storage. Without develop its trunk is main or master. Without production its releases go into master or main, from the trunk or from a release or hotfix branch.")
